...
class Calculator:
    ...
    def bind_keys(self):
        ...
        for key in self.operations:
            self.window.bind(key, lambda event, operators=key: self.append_operators(operators))
    ...
    def create_digit_buttons(self):
        for digits, grid_values in self.digits.items():
    ...
    def create_operator_buttons(self):
        ...
        for operator, symbol in self.operations.items():
            button = tk.Button(self.buttons_frame, text=symbol, bg=OFF_WHITE, fg=LABEL_COLOR, font=DEFAULT_FONT_STYLE, borderwidth=0, command=lambda x=operator: self.append_operators(x))
